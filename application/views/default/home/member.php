<div class="row">
	...
	<?php foreach($data['news'] as $news): ?>
		...
		<div class="<?php echo $class; ?> news-item news-item-<?php echo $item_count; ?>">
			<?php if($item_count == 1): ?><h3><a href="<?php echo base_url('index.php/news/detail/' . $news->idpost); ?>"><?php echo $news->title; ?></a></h3><?php else: ?><h4><a href="<?php echo base_url('index.php/news/detail/' . $news->idpost); ?>"><?php echo $news->title; ?></a></h4><?php endif; ?>
			...
			<p class="alert alert-warning author-info">
				...
				echo (
				     ...
				     . ($author ? $author[0]->display_name : $author[0]->first_name . ' ' . substr($author[0]->last_name, 0, 1));
				?>
				<i class="fa fa-fw fa-calendar-o"></i> <?php echo date('d M, Y h:i a', strtotime($news->add_date)); ?>
			</p>
			<div class="news-excerpt">
				<?php echo substr(strip_tags($news->body), 0, $length); ?> ... <a href="<?php echo base_url('index.php/news/detail/' . $news->idpost); ?>">read more <i class="fa fa-fw fa-external-link"></i></a>
			</div>
		</div>
